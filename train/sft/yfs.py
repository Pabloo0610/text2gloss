...
finetune_model_path='../../data/g2t_model/t2g_new_r_32/checkpoint-12400'
...
prompt = '''你是一个优秀的手语翻译助手。你的任务是首先理解给定的汉语自然语言文本的含义, 然后将其翻译成对应的手语gloss序列, 遵循以下规则:
...
11.为保持简洁,gloss序列字数应当不超过给定原文文本字数的60%。
12.有一些词为多义词,gloss序列中体现为该词后面接"1"或者"2"。例如:"你可以帮我记录一下吗?"中的"记录"翻译为"记录1",而"你可以帮我打印一下这份记录吗?"中的"记录"翻译为"记录2"。
请翻译以下汉语文本: '''
...
for line in lines:
    text = line.strip()
        # row = f"<s>Human: {prompt}你要翻译的句子是：{text}\n</s><s>Assistant: {gloss} </s>"
    input_text = f"<s>Human: {prompt}{text} </s><s>Assistant: "
