...
finetune_model_path='../../data/g2t_model/t2g_match_r_32'
...
自然语言文本: '''
df = pd.read_csv("../../data/20kdata/t2g_test_match.csv")
for ids,rows in df.iterrows():
#with open('../../data/20kdata/yfs.txt', 'r', encoding='utf-8') as f:
    #lines = f.readlines()
#for line in lines:
    #text = line.strip()
    input_text = rows['text'].split('</s><s>Assistant:')[0]+'</s><s>Assistant: '
